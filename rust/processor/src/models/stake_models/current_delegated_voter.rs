...
use aptos_indexer_protos::transaction::v1::{
    transaction::TxnData,
    write_set_change::Change as WriteSetChange,
...
pub type PKtoVoteDelegation = HashMap<CurrentDelegatedVoterPK, (Option<String>, Option<String>, String)>;
...
impl CurrentDelegatedVoter {
    pub fn get_delegation_pool_address_by_table_handle (
        ...
        while retried < QUERY_RETRIES {
            ...
            match CurrentDelegatedVoterQuery::get_by_table_handle (conn, table_handle) {
                Ok(current_delegated_voter_query_result) => return Ok(current_delegated_voter_query_result.delegation_pool_address),
                Err(_) => {
    ...
    pub fn get_existance_by_pk (
        ...
        while retried < QUERY_RETRIES {
            retried += 1;
            match CurrentDelegatedVoterQuery::get_by_pk (conn, delegation_pool_address, delegator_address) {
                Ok(current_delegated_voter_query_result) => return Ok(true),
                Err(_) => {
                    std::thread::sleep(std::time::Duration::from_millis(QUERY_RETRY_DELAY_MS));
                },
            }
    ...
    pub fn get_pk_from_write_table_item (
        ...
        if let Some(pool_balance) = active_pool_to_staking_pool.get(&table_handle) {
            ...
            return Ok(Some((delegation_pool_address, delegator_address)));
        } else {
            Ok(None)
        }
    }

    pub fn from_transaction(transaction: &TransactionPB, conn: &mut PgPoolConnection) -> anyhow::Result<CurrentDelegatedVoterMap> {
        ...
        if let TxnData::User(user_txn) = txn_data {
            ...
            for wsc in &transaction_info.changes {
                if let WriteSetChange::WriteResource(write_resource) = wsc.change.as_ref().unwrap() {
                    if let Some(DelgationVoteGovernanceRecordsResource::GovernanceRecords(inner)) =
                        DelgationVoteGovernanceRecordsResource::from_write_resource(write_resource, txn_version)?
                    {
                        ...
                        vote_delegation_handle_to_pool_address.insert(
                            vote_delegation_handle,
                            delegation_pool_address.clone(),
                        );
                    }
                } 
            }
            ...
            for wsc in &transaction_info.changes {
                if let WriteSetChange::WriteTableItem(write_table_item) = wsc.change.as_ref().unwrap() {
                    let table_item_data = write_table_item.data.as_ref().unwrap();
                    let table_handle = write_table_item.handle;
                    if let Some(VoteDelegationTableItem::VoteDelegation(inner)) =
                        ...
                        let delegator_address =
                            standardize_address(&write_table_item.key.to_string());
                        let voter = standardize_address(&inner.get_voter());
                        let pending_voter = standardize_address(&inner.get_pending_voter());
                            ...
                            vote_delegation_handle_to_pool_address.get(&table_handle)
                        {
                            ...
                            pk_to_vote_delegation.insert(
                                ...
                                (Some(voter), Some(pending_voter), table_handle.clone()),
                            );
                        } else {
                            match Self::get_delegation_pool_address_by_table_handle(
                                ...
                                Ok(delegation_pool_address_from_query_result) => {
                                    let current_delegated_voter = CurrentDelegatedVoter {
                                        delegator_address: delegator_address.clone(),
                                        delegation_pool_address: delegation_pool_address_from_query_result.clone(),
                                        voter: Some(voter.clone()),
                                        pending_voter: Some(pending_voter.clone()),
                                        transaction_timestamp: txn_timestamp,
                                        last_transaction_version: txn_version,
            ...
            for wsc in &transaction.info.as_ref().unwrap().changes {
                if let WriteSetChange::WriteResource(write_resource) = wsc.change.as_ref().unwrap() {
                    if let Some(map) = Self::get_active_pool_to_staking_pool_mapping(
                        write_resource,
                        txn_version,
                    )? {
            ...
            for wsc in &transaction.info.as_ref().unwrap().changes {
                if let WriteSetChange::WriteTableItem(write_table_item) = wsc.change.as_ref().unwrap() {
                    if let Some(pk) = Self::get_pk_from_write_table_item(
                        ...
                        {
                            match Self::get_existance_by_pk(
                                conn,
                                &pk.0,
                                &pk.1,
                            ) {
                                Ok(pk_exist) => {
                                    ...
                                    current_delegated_voter_map.insert(pk, new_current_delegated_voter);
                                }
                                Err(_) => {
                                    tracing::error!(
                                        transaction_version = txn_version,
