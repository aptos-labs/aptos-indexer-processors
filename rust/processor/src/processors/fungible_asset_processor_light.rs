...
pub struct FungibleAssetProcessorLight {
    connection_pool: PgDbPool,
}

impl FungibleAssetProcessorLight {
    pub fn new(connection_pool: PgDbPool) -> Self {
        Self { connection_pool }
    }
}

impl Debug for FungibleAssetProcessorLight {
...
#[async_trait]
impl ProcessorTrait for FungibleAssetProcessorLight {
    fn name(&self) -> &'static str {
        ProcessorName::FungibleAssetProcessorLight.into()
    }
    ...
    async fn process_transactions(
        ...
        let (fungible_asset_activities, fungible_asset_metadata, current_fungible_asset_balances) =
    ...
    for txn in transactions {
        ...
        let txn_data = match txn.txn_data.as_ref() {
            ...
            None => {
                ...
                PROCESSOR_UNKNOWN_TYPE_COUNT
                    .with_label_values(&["FungibleAssetProcessorLight"])
                    .inc();
                continue;
            },
        };
        ...
        for (index, wsc) in transaction_info.changes.iter().enumerate() {
            if let Change::WriteResource(write_resource) = wsc.change.as_ref().unwrap() {
                if let Some((_, current_balance, event_to_coin)) =
