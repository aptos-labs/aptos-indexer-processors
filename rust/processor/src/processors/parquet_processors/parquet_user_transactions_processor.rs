...
use crate::{
    ...
    db::common::models::user_transactions_models::{
        parquet_signatures::Signature, parquet_user_transactions::UserTransaction,
    },
...
pub struct ParquetUserTransactionsProcessor {
    ...
    signatures_sender: AsyncSender<ParquetDataGeneric<Signature>>,
}
...
impl ParquetUserTransactionsProcessor {
    pub fn new(
        ...
        let signatures_sender: AsyncSender<ParquetDataGeneric<Signature>> =
            create_parquet_handler_loop::<Signature>(
                new_gap_detector_sender.clone(),
                ProcessorName::ParquetUserTransactionsProcessor.into(),
                config.bucket_name.clone(),
                config.bucket_root.clone(),
...
impl Debug for ParquetUserTransactionsProcessor {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "ParquetProcessor {{ capacity of user transactions channel: {:?}, capacity of signatures channel: {:?}}}",
            &self.user_transactions_sender.capacity(),
            &self.signatures_sender.capacity(),
        )
    }
}
    ...
    async fn process_transactions(
        ...
        for txn in &transactions {
            ...
            if let TxnData::User(inner) = txn_data {
                let (user_transaction, sigs) = UserTransaction::from_transaction(
                    inner,
                ...
                transaction_version_to_struct_count
                    .entry(txn_version)
                    .and_modify(|e| *e += user_transactions.len() as i64)
                    .or_insert(user_transactions.len() as i64);
        ...
        let signature_parquet_data = ParquetDataGeneric { data: signatures };

        self.user_transactions_sender
            .send(user_transaction_parquet_data)
            .await
            .context("Failed to send to parquet manager")?;
        self.signatures_sender
            .send(signature_parquet_data)
            .await
            .context("Failed to send to parquet manager")?;
