#[cfg(test)] // Only compile this module in test mode
pub mod fa_test_helpers {
    ...
    pub fn load_data(conn: &mut PgConnection, txn_version: &str) -> Result<HashMap<String, Value>> {
        ...
        let fungible_asset_metadata_json =
            serde_json::to_string_pretty(&all_fungible_asset_metadata)?;
        result_map.insert(
        ...
        let fungible_asset_balances_json =
            serde_json::to_string_pretty(&all_fungible_asset_balances)?;
        result_map.insert(
        ...
        let current_fungible_asset_balances_result =
            cfab_dsl::current_fungible_asset_balances_legacy
